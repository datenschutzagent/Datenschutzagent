...
def _parse_args() -> argparse.Namespace:
    ...
    users_sub.add_parser("list", help="List all users (id, display_name, email, role, oidc_sub)")
    set_role_p = users_sub.add_parser("set-role", help="Set user role (viewer | editor | admin)")
    ...
    config_sub.add_parser("check", help="Run connection checks (Ollama, Weaviate, Postgres, MinIO, Redis)")
...
async def _cmd_users_list() -> int:
    ...
    if not users:
        ...
        return 0
    for u in users:
        oidc = (u.oidc_sub or "")[:40] + ("..." if (u.oidc_sub and len(u.oidc_sub) > 40) else "")
...
async def _cmd_config_show() -> int:
    ...
    data = {
        ...
        "storage_local_path": s.storage_local_path if s.storage_backend == "local" else None,
        "s3_configured": bool(s.s3_endpoint_url and s.s3_access_key and s.s3_secret_key),
