...
class CaseModel(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, default=CaseStatus.INTAKE
    )
    ...
    auto_run_checks: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    archived_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True, default=None
    )
    retention_months: Mapped[int | None] = mapped_column(
        Integer, nullable=True, default=None
    )
    recheck_interval_days: Mapped[int | None] = mapped_column(
        Integer, nullable=True, default=None
    )
    ...
    documents: Mapped[list["DocumentModel"]] = relationship(
        "DocumentModel", back_populates="case", cascade="all, delete-orphan"
    )
    findings: Mapped[list["FindingModel"]] = relationship(
        "FindingModel", back_populates="case", cascade="all, delete-orphan"
    )
