...
async def _compute_funnels(db: AsyncSession, department: str | None) -> list[dict]:
    ...
    dsr_funnel_q = text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text
        f"""
        WITH ordered AS (
            SELECT a.request_id,
                   a.event_type,
    ...
    breach_funnel_q = text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text
        f"""
        WITH ordered AS (
            SELECT a.breach_id,
                   a.event_type,
...
async def velocity_stats(
    ...
    dsr_q = text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text
        f"""
    ...
    breach_q = text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text
        f"""
    ...
    find_q = text(  # nosemgrep: python.sqlalchemy.security.audit.avoid-sqlalchemy-text
        f"""
