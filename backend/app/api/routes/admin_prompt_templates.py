...
async def create_prompt_template(
    ...
    if not version:
        ...
        if existing.scalar_one_or_none() is not None:
            raise HTTPException(
                status_code=409, detail="This key and version already exist"
            )
    if body.set_active:
        result = await db.execute(
            select(PromptTemplateModel).where(PromptTemplateModel.key == body.key)
        )
            ...
            (
                await db.execute(
                    select(PromptTemplateModel).where(
                        PromptTemplateModel.key == template.key
                    )
                )
            )
            .scalars()
