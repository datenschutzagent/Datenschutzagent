...
from tests.factories import create_breach
...
async def test_create_breach_returns_id(client):
    breach = await create_breach(client, title="Neues Datenleck")
    assert "id" in breach
...
async def test_create_breach_has_notification_deadline(client):
    breach = await create_breach(client, discovered_at="2026-04-14T10:00:00")
...
async def test_get_breach_by_id(client):
    breach = await create_breach(client, title="Get-by-ID Datenpanne")
    breach_id = breach["id"]
...
async def test_list_breaches_includes_created(client):
    breach = await create_breach(client, title="List-Test-Datenpanne")
...
async def test_update_breach_status(client):
    breach = await create_breach(client)
    breach_id = breach["id"]
...
async def test_delete_breach(client):
    breach = await create_breach(client, title="Zu löschende Datenpanne")
    breach_id = breach["id"]
