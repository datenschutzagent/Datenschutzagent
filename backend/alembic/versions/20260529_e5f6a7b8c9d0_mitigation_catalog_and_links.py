...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE IF EXISTS avv_contracts DROP COLUMN IF EXISTS inherent_risk_level"
    )
    op.execute(
        "ALTER TABLE IF EXISTS avv_contracts DROP COLUMN IF EXISTS inherent_risk_score"
    )
