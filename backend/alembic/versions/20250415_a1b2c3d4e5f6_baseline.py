...
def upgrade() -> None:
    # ---- Documents ----
    op.execute("ALTER TABLE documents ADD COLUMN IF NOT EXISTS extraction_method VARCHAR(20)")
    op.execute("ALTER TABLE documents ADD COLUMN IF NOT EXISTS extraction_status VARCHAR(20)")
    ...
    # ---- Cases ----
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS processing_context VARCHAR(80) NULL")
    op.execute(
    ...
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS recheck_interval_days INTEGER")
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS last_rechecked_at TIMESTAMPTZ")
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS last_notified_at TIMESTAMPTZ")
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ")
    op.execute("ALTER TABLE cases ADD COLUMN IF NOT EXISTS auto_run_checks BOOLEAN NOT NULL DEFAULT false")

    # ---- DSR requests ----
    op.execute("ALTER TABLE dsr_requests ADD COLUMN IF NOT EXISTS draft_response TEXT")
    op.execute("ALTER TABLE dsr_requests ADD COLUMN IF NOT EXISTS last_notified_at TIMESTAMPTZ")
    op.execute(
    ...
    op.execute("ALTER TABLE data_breaches ADD COLUMN IF NOT EXISTS draft_notification TEXT")
    op.execute("ALTER TABLE data_breaches ADD COLUMN IF NOT EXISTS last_notified_at TIMESTAMPTZ")
    ...
    op.execute("ALTER TABLE avv_contracts ADD COLUMN IF NOT EXISTS risk_level VARCHAR(20)")
    op.execute("ALTER TABLE avv_contracts ADD COLUMN IF NOT EXISTS risk_assessment JSONB")
    op.execute("ALTER TABLE avv_contracts ADD COLUMN IF NOT EXISTS risk_assessed_at TIMESTAMPTZ")
    op.execute("ALTER TABLE avv_contracts ADD COLUMN IF NOT EXISTS last_notified_at TIMESTAMPTZ")
